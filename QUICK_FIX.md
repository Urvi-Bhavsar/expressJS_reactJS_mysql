...
📊 Health check available at: `${process.env.VITE_API_URL}/health`
...
# Check status
curl `${process.env.VITE_API_URL}/health`
```
...
```bash
curl -X POST `${process.env.VITE_API_URL}/create` \
...
```bash
curl `${process.env.VITE_API_URL}/get-employee-details`
```
